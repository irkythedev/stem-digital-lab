...
const copy = {
  zh: {
    ...
    ai: 'AI 学习助手',
    aiIntro: '顶栏「AI 学习助手」入口可辅助解释数理化知识。您需自行配置 AI 服务商（支持 DeepSeek、通义千问、Kimi、智谱 GLM、豆包等）的 API Key，本站不提供、不代购、不收取任何费用。支持问答历史（仅保存在本机浏览器，可随时清除，支持按科目/知识点筛选），回答末尾会推荐 3 个追问并支持「换一批」；回答可一键朗读，数学公式、化学式按规范读法读出而非逐字符；「考考你」可让 AI 基于当前知识点批量出单选题（可选题量与基础知识/进阶提升/易混淆辨析角度，支持每题限时），作答后即时判分与讲解，错题自动记入错题集；不提供自由输入框。',
  ...
  en: {
    ...
    ai: 'AI Assistant',
    aiIntro: 'The AI assistant (header entry) helps explain math / physics / chemistry. You configure your own API key (DeepSeek, Qwen, Kimi, Zhipu GLM, Doubao and more); this site provides no key, sells nothing and charges nothing. Q&A history is stored only in your browser, clearable anytime, filterable by subject or topic; each answer recommends 3 follow-up questions with a "refresh" option; answers can be read aloud at one click, with math formulas and chemical names read in standard spoken form instead of raw characters; "Quiz" lets the AI batch-generate single-choice questions from the current topic (pick the count and an angle — basic knowledge / advanced / easy-to-confuse concepts — with an optional per-question time limit), scoring each instantly with an explanation and auto-saving mistakes to a mistake collection — there is no free-text input.',
