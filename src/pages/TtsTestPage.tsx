          ...
          return (
            ...
            >
              <button
                ...
                className="shrink-0 w-9 h-9 rounded-full border border-[var(--fg)] flex items-center justify-center text-[var(--fg)] hover:bg-[var(--accent-light)] disabled:opacity-50 transition-colors"
                aria-label={`播放 ${c.label}`}
              >
                {isPlaying && playState === 'loading' ? (
                  <span className="animate-spin inline-block w-3.5 h-3.5 border-2 border-[var(--fg)] border-t-transparent rounded-full" />
              ...
              <div className="flex-1 min-w-0 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[10px] text-[var(--muted)] mb-0.5">朗读文本</div>
                  <div className="text-sm text-[var(--fg)] break-words">{spoken}</div>
                </div>
              </div>
            </div>
          );
